...
var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; /* eslint-disable react/no-render-return-value */
...
describe('SLDSTooltip: ', function () {
	...
	var defaultTrigger = _react2.default.createElement(_button2.default, { label: 'Hover me for tooltip' });
	...
	describe('component basic props render', function () {
		...
		beforeEach(function () {
			createBody();
			rootNode = generateTooltip(_extends({}, defaultProps, {
				align: 'top'
			}), defaultTrigger);
		});

		it('renders the content as assistive text', function () {
			var span = findRenderedDOMComponentWithClass(rootNode, 'slds-assistive-text');
			(0, _chai.expect)(span.textContent).to.equal('This is more info. blah blah.');
		});
	...
	describe('Custom props work as expected', function () {
		it('isOpen is false', function (done) {
			var rootNode = generateTooltip(_extends({}, defaultProps, {
				isOpen: false
			}), defaultTrigger);
		...
		it('isOpen is true', function (done) {
			...
			generateTooltip(_extends({}, defaultProps, {
