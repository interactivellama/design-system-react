define(['react', 'prop-types', 'chai', 'chai-enzyme', 'enzyme', 'lodash.assign', '../../../tests/enzyme-helpers', '../../../components/combobox', '../../../components/icon', '../../../../components/combobox/filter', '../../../utilities/key-code', '../../../utilities/letter-key-code', '../../../../components/icon-settings'], function (_react, _propTypes, _chai, _chaiEnzyme, _enzyme, _lodash, _enzymeHelpers, _combobox, _icon, _filter, _keyCode, _letterKeyCode, _iconSettings) {
	'use strict';
  ...
  */
    ...
    */
			...
			it('has aria-haspopup, aria-expanded is false when closed, aria-expanded is true when open, ', function () {
				wrapper = (0, _enzyme.mount)(_react2.default.createElement(DemoComponent, { multiple: true }), { attachTo: mountNode });
				var nodes = getNodes({ wrapper: wrapper });
				(0, _chai.expect)(nodes.combobox.node.getAttribute('aria-haspopup')).to.equal('listbox');
				// closed
				(0, _chai.expect)(nodes.combobox.node.getAttribute('aria-expanded')).to.equal('false');
				// open
			...
			it('menu filters to second item, menu listbox menu item 2 aria-selected is true, input activedescendent has item 2 id, after pressing down arrow, enter selects item 2', function () {
				wrapper = (0, _enzyme.mount)(_react2.default.createElement(DemoComponent, { multiple: true, isOpen: true }), { attachTo: mountNode });
				var nodes = getNodes({ wrapper: wrapper });
				...
				(0, _chai.expect)(nodes.input.node.getAttribute('aria-activedescendant')).to.equal(defaultProps.id + '-listbox-option-2');
				// select
				nodes.input.simulate('keyDown', _keyCode.keyObjects.ENTER);
				nodes = getNodes({ wrapper: wrapper });
			...
			it('Selected Listbox: remove initial first pill, remove third initial item, cycles focus (first to last), removes last and initial fifth pill, cycles focus (last to first), remove inital second and fourth pill', function (done) {
				...
				getSelectedListboxPills({ nodes: nodes, index: 0 }).simulate('keyDown', _keyCode.keyObjects.DELETE);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[1].label);
				getSelectedListboxPills({ nodes: nodes, index: 0 }).simulate('keyDown', _keyCode.keyObjects.RIGHT);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[2].label);
				getSelectedListboxPills({ nodes: nodes, index: 1 }).simulate('keyDown', _keyCode.keyObjects.DELETE);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[3].label);
				getSelectedListboxPills({ nodes: nodes, index: 1 }).simulate('keyDown', _keyCode.keyObjects.LEFT);
				getSelectedListboxPills({ nodes: nodes, index: 0 }).simulate('keyDown', _keyCode.keyObjects.LEFT);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[4].label);
				getSelectedListboxPills({ nodes: nodes, index: 2 }).simulate('keyDown', _keyCode.keyObjects.DELETE);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[3].label);
				getSelectedListboxPills({ nodes: nodes, index: 1 }).simulate('keyDown', _keyCode.keyObjects.RIGHT);
				(0, _chai.expect)(getFocusedPillLabel()).to.equal(accountsWithIcon[1].label);
				getSelectedListboxPills({ nodes: nodes, index: 0 }).simulate('keyDown', _keyCode.keyObjects.DELETE);
				getSelectedListboxPills({ nodes: nodes, index: 0 }).simulate('keydown', _keyCode.keyObjects.DELETE);
			});
		...
		describe('Variant-specific', function () {
			...
			it('Limit to pre-defined choices', function () {
				wrapper = (0, _enzyme.mount)(_react2.default.createElement(DemoComponent, { multiple: true, predefinedOptionsOnly: true }), { attachTo: mountNode });
				var nodes = getNodes({ wrapper: wrapper });
				nodes.input.simulate('focus');
			...
			it('Inline Single Selection Remove selection', function () {
				wrapper = (0, _enzyme.mount)(_react2.default.createElement(DemoComponent, { variant: 'inline-listbox' }), { attachTo: mountNode });
				var nodes = getNodes({ wrapper: wrapper });
		...
		describe('Optional Props', function () {
			...
			it('Displays No match found', function () {
				wrapper = (0, _enzyme.mount)(_react2.default.createElement(DemoComponent, { isOpen: true }), { attachTo: mountNode });
				var nodes = getNodes({ wrapper: wrapper });
				nodes.input.simulate('focus');
