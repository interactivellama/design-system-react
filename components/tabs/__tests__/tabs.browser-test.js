...
function _objectWithoutProperties(obj, keys) { var target = {}; for (var i in obj) { if (keys.indexOf(i) >= 0) continue; if (!Object.prototype.hasOwnProperty.call(obj, i)) continue; target[i] = obj[i]; } return target; } // Import your external dependencies
   ...
   */
	...
	render: function render() {
		var _props = this.props,
		    ...
		    id = _props.id,
		    attributes = _objectWithoutProperties(_props, ['className', 'id']);
		...
		return _react2.default.createElement(
			'div',
			{
				className: (0, _classnames2.default)('slds-m-top--large', '' + COMPONENT_CSS_CLASSES.wrapper)
			},
			_react2.default.createElement(
				...
				_react2.default.createElement(
					...
					_react2.default.createElement(
						'p',
						null,
						'This one can not be selected from the tabs list because this example provides a custom ',
						_react2.default.createElement(
							'code',
							null,
							'onSelct'
						),
						' function that retuns false when it is run, preventing the component\u2019s built-in handler from running, and thus the tab is never selected.'
					),
					_react2.default.createElement(
						'p',
						null,
						...
						' still see the panel if you hide the other tabs, because the tab/panel are not ',
						_react2.default.createElement(
							'em',
							null,
							'disabled'
					...
					_react2.default.createElement(
						'p',
						null,
						'In other words, this should not be taken as an example of how to be sneaky about disabling tab selection, but rather that you can ',
						_react2.default.createElement(
							'strong',
							null,
							'do stuff'
						),
						' when a tab is selected by sending it a custom ',
						_react2.default.createElement(
							'code',
							null,
							'onSelect'
...
describe('Tabs', function () {
	...
	describe('Default structure and CSS', function () {
		...
		it('Has a main wrapper with the proper class name.', function () {
			var myTabsWrapper = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.wrapper);
		...
		it('Has exactly one (1) tabs component, and has with the proper class name.', function () {
			var myTabs = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.base);
		...
		it('Has exactly one (1) nav component, and has with the proper class name.', function () {
			var myTabsNav = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.nav);
		...
		it('Nav component builds proper ID (' + id + '-tabs__nav) because it inherits Tabs id property and appends "-slds-tabs__nav" to it.', function () {
			var myTabsNav = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.nav);
			(0, _chai.expect)(myTabsNav).attr('id').to.equal(id + '-slds-tabs__nav');
		});

		it('Has exactly four (4) <Tab /> components, each with the proper class name (' + COMPONENT_CSS_CLASSES.item + ').', function () {
			var myTabsListItems = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item);
		...
		it('Has only one (1) tab with ".slds-disabled" class on it.', function () {
			var myTabsListItem = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item + '.slds-disabled');
		...
		it('Tab components have proper ID attributes because they inherit the Tabs "id" property and append "-slds-tabs--tab-<index>" to it.', function () {
			this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item).forEach(function (node, index) {
				(0, _chai.expect)(node).to.have.attr('id', id + '-slds-tabs--tab-' + index);
			});
		});
		...
		it('TabPanel components have proper ID attributes because they inherit the Tabs "id" property and append "-slds-tabs--panel-<index>" to it.', function () {
			this.wrapper.find('.' + COMPONENT_CSS_CLASSES.panel).forEach(function (node, index) {
				(0, _chai.expect)(node).to.have.attr('id', id + '-slds-tabs--panel-' + index);
			});
		});
		...
		it('Has the same number of tabs as panels.', function () {
			var myTabsListItems = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item);
			var myTabsPanels = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.content);
			(0, _chai.expect)(myTabsListItems).to.have.length(4);
   ...
   */
		...
		it('Tab components have proper "aria-controls" attribute because they inherit Tabs ID property and append "-slds-tabs--panel-<index>" to it.', function () {
			this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item).forEach(function (node, index) {
				(0, _chai.expect)(node).to.have.attr('aria-controls', id + '-slds-tabs--panel-' + index);
			});
		});
		...
		it('TabPanel components have proper "aria-labelledby" attribute because they inherit Tabs ID property and append "-slds-tabs--tab-<index>" to it.', function () {
			this.wrapper.find('.' + COMPONENT_CSS_CLASSES.panel).forEach(function (node, index) {
				(0, _chai.expect)(node).to.have.attr('aria-labelledby', id + '-slds-tabs--tab-' + index);
			});
		});
	...
	describe('Interactions click', function () {
		...
		it('New panel renders when a tab is clicked ', function () {
			var myTabsListItems = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item);
			var myFirstPanel = this.wrapper.find('#' + id + '-slds-tabs--panel-0');
			var myThirdPanel = this.wrapper.find('#' + id + '-slds-tabs--panel-2');
	...
	describe('Interactions disabled', function () {
		...
		it('Disabled tab does not reveal new content ', function () {
			var myTabsListItems = this.wrapper.find('.' + COMPONENT_CSS_CLASSES.item);
			var myFirstPanel = this.wrapper.find('#' + id + '-slds-tabs--panel-0');
			var mySecondPanel = this.wrapper.find('#' + id + '-slds-tabs--panel-1');
