define(['react', 'react-dom', 'lodash.assign', 'react-addons-test-utils', 'chai', '../../lookup', '../../icon-settings', '../../lookup/header', '../../lookup/footer'], function (_react, _reactDom, _lodash, _reactAddonsTestUtils, _chai, _lookup, _iconSettings, _header, _footer) {
	'use strict';
	...
	describe('SLDSLookup: ', function () {
		...
		var defaultProps = {
			...
			options: [{ label: 'Paddy"s Pub' }, { label: 'Tyrell Corp' }, { label: 'Paper St. Soap Company' }, { label: 'Nakatomi Investments' }, { label: 'Acme Landscaping' }, { label: 'Acme Construction' }]
		};
		...
		var getLookupWithHeader = function getLookupWithHeader() {
			var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { headerRenderer: _header2.default };
			return _react2.default.createElement(_lookup2.default, (0, _lodash2.default)({}, defaultProps, props));
		};
		var getLookupWithSelection = function getLookupWithSelection() {
			var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { selectedItem: 1 };
			return _react2.default.createElement(_lookup2.default, (0, _lodash2.default)({}, defaultProps, props));
		};
		...
		describe('component basic props render', function () {
			...
			it('isOpen=true renders open dropdown', function () {
				var lookup = generateLookup(getLookup({ isOpen: true }));
		...
		describe('accessibility aria attributes pass', function () {
			...
			it('LookupWithSelection - aria-expanded is true when deleting selection', function () {
				...
				_reactAddonsTestUtils2.default.Simulate.keyDown(deleteBtn, { key: 'Down', keyCode: 46, which: 46 });
				var ariaExpanded = lookup.getElementsByTagName('input')[0].getAttribute('aria-expanded');
				(0, _chai.expect)(ariaExpanded).to.equal('true');
			});
		});
		...
		describe('selecting item works', function () {
			...
			it('closes lookup menu on esc', function () {
				...
				_reactAddonsTestUtils2.default.Simulate.click(input);
				_reactAddonsTestUtils2.default.Simulate.keyDown(input, { key: 'Down', keyCode: 40, which: 40 });
				_reactAddonsTestUtils2.default.Simulate.keyDown(input, { key: 'Esc', keyCode: 27, which: 27 });
				var ariaExpanded = input.getAttribute('aria-expanded');
				(0, _chai.expect)(ariaExpanded).to.equal('false');
			});
			...
			it('isOpen=false prevents dropdown from opening', function () {
				var lookup = generateLookup(getLookup({ isOpen: false }));
				var input = lookup.getElementsByTagName('input')[0];
		...
		describe('custom filter', function () {
			...
			beforeEach(function () {
				lookup = generateLookup(getLookup({ filterWith: function filterWith(text, i) {
