define(['react', 'react-dom', 'lodash.assign', 'react-addons-test-utils', 'chai', '../../modal', '../../icon-settings'], function (_react, _reactDom, _lodash, _reactAddonsTestUtils, _chai, _modal, _iconSettings) {
	'use strict';
	...
	describe('SLDSModal: ', function () {
		...
		var renderModal = function renderModal(modalInstance) {
			...
			var opener = _react2.default.createElement(
				'button',
				null,
		...
		describe('Open modal', function () {
			...
			it('renders a noscript', function () {
				var renderedNoScriptNode = _reactDom2.default.findDOMNode(cmp);
				(0, _chai.expect)(renderedNoScriptNode.firstChild.tagName).to.equal('NOSCRIPT');
			});
		...
		describe('Open with Prompt and Footer', function () {
			...
			beforeEach(function () {
				var feet = _react2.default.createElement(
					'div',
					{ className: 'toes' },
					'Toes'
				);
		...
		describe('Keyboard behavior', function () {
			...
			it('first tab focuses close button', function (done) {
				setTimeout(function () {
					...
					setTimeout(function () {
						(0, _chai.expect)(document.activeElement.className).to.include('slds-modal__close');
						done();
					}, 200);
				}, 200);
			});

			it('enter on close button works', function () {
				// TODO: simulate enter on close button and modal is undefined
			});

			it('traps focus inside Modal', function () {
