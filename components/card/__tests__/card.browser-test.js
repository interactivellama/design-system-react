define(['react', 'react-dom', 'lodash.assign', 'chai', '../../card', '../../card/filter', '../../media-object', '../../icon', '../../icon-settings'], function (_react, _reactDom, _lodash, _chai, _card, _filter, _mediaObject, _icon, _iconSettings) {
	'use strict';
	...
	describe('Card: ', function () {
		...
		var renderCard = function renderCard(instance) {
			return function () {
				...
				this.component = _reactDom2.default.render(_react2.default.createElement(
		...
		var getCard = function getCard(dom) {
			return dom.querySelector('.' + cssClasses.base);
		};
		...
		var getBody = function getBody(dom) {
			return getCard(dom).querySelectorAll('#' + requiredProps.id + cardIdSuffixes.body)[0];
		};
		...
		describe('Default Structure', function () {
			...
			it('has the correct heading text', function () {
				var heading = getHeader(this.dom).querySelectorAll('#' + requiredProps.id + cardIdSuffixes.heading)[0];
				heading.textContent = requiredProps.heading;
			});
		});

		// Optional props
		var renderFooterContents = _react2.default.createElement('span', { id: 'sampleFooter' });
		var renderHeaderActions = _react2.default.createElement('span', { id: 'sampleHeaderActions' });
		...
		describe('Optional Structure', function () {
			...
			it('has an icon', function () {
				var header = getHeader(this.dom);
				var icon = header.querySelectorAll('.' + _mediaObject.cssClasses.figure)[0];
